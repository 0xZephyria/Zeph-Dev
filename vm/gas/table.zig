...
// Gas cost table for RV32EM instructions and syscalls.
// Costs are calibrated to approximate real CPU cycle costs.

const decoder = @import("../core/decoder.zig");

/// Gas costs for base RV32EM instructions (indexed by opcode)
...
pub fn instructionCost(insn: decoder.Instruction) u64 {
    return switch (insn) {
        .r_type => |r| rTypeCost(r),
        .i_type => InstructionGas.ALU_IMM, // covers ADDI, SLTI, loads, JALR
        .s_type => InstructionGas.STORE,
        .b_type => InstructionGas.BRANCH,
        .u_type => InstructionGas.LUI, // LUI and AUIPC same cost
        .j_type => InstructionGas.JAL,
...
test "MUL R-type costs 2 gas" {
    const r = decoder.RType{ .rd = 1, .rs1 = 2, .rs2 = 3, .funct3 = decoder.Funct3.MUL, .funct7 = decoder.Funct7.MULDIV, .word_op = false };
    try testing.expectEqual(@as(u64, 2), rTypeCost(r));
}

test "DIV R-type costs 5 gas" {
    const r = decoder.RType{ .rd = 1, .rs1 = 2, .rs2 = 3, .funct3 = decoder.Funct3.DIV, .funct7 = decoder.Funct7.MULDIV, .word_op = false };
