...
pub const ForgeVM = struct {
    ...
    current_word: u32 = 0,
    is_polkavm: bool = false,
    heap_ptr: u32 = 0x0002_0000,
    ...
    pub fn step(self: *ForgeVM) void {
        ...
        // 1. Check PC bounds
        if (self.pc >= self.codeLen or self.pc & 1 != 0) {
            self.status = .fault;
        ...
        // 2. Fetch instruction word (supporting RVC)
        const h0 = self.memory.loadHalfword(self.pc) catch {
            self.status = .fault;
            self.faultReason = "Failed to fetch instruction (halfword)";
            return;
        };
        const is_rvc = (h0 & 3) != 3;
        var word: u32 = h0;
        var pc_increment: u32 = 4;
        if (is_rvc) {
            word = decoder.decompress(h0) orelse {
                self.status = .fault;
                self.faultReason = "Illegal compressed instruction";
                return;
            };
            pc_increment = 2;
        } else {
            const h1 = self.memory.loadHalfword(self.pc + 2) catch {
                self.status = .fault;
                self.faultReason = "Failed to fetch second halfword of 32-bit instruction";
                return;
            };
            word = h0 | (@as(u32, h1) << 16);
            pc_increment = 4;
        }
        self.current_word = word;
        if (comptime enable_step_logging) {
    ...
    // -------------------------------------------------------------------
    // M-extension helpers: schoolbook multiply-high
    // -------------------------------------------------------------------
    ...
    fn execCustomPolkaVM(self: *ForgeVM, c: decoder.CustomType) void {
        _ = c;
        const inst = self.current_word;
        const funct3 = (inst >> 12) & 7;

        switch (funct3) {
            0 => { // FUNC3_ECALLI -> dynamic import call
                self.memory.checkAccess(self.pc + 4, 8, .read) catch {
                    self.status = .fault;
                    self.faultReason = "Failed to load dynamic import metadata pointer";
                    return;
                };
                const metadata_ptr = std.mem.readInt(u64, self.memory.backing[@intCast(self.pc + 4)..][0..8], .little);


                const version = self.memory.loadByte(metadata_ptr) catch {
                    self.status = .fault;
                    self.faultReason = "Failed to load dynamic import metadata version";
                    return;
                };
                _ = version;

                self.memory.checkAccess(metadata_ptr + 5, 4, .read) catch {
                    self.status = .fault;
                    self.faultReason = "Failed to load dynamic import symbol length";
                    return;
                };
                const symbol_len = std.mem.readInt(u32, self.memory.backing[@intCast(metadata_ptr + 5)..][0..4], .little);

                self.memory.checkAccess(metadata_ptr + 9, 8, .read) catch {
                    self.status = .fault;
                    self.faultReason = "Failed to load dynamic import symbol pointer";
                    return;
                };
                const symbol_ptr = std.mem.readInt(u64, self.memory.backing[@intCast(metadata_ptr + 9)..][0..8], .little);


                const symbol = self.memory.getSlice(symbol_ptr, symbol_len) catch {
                    self.status = .fault;
                    self.faultReason = "Failed to read dynamic import symbol string";
                    return;
                };

                const syscallId = mapNameToSyscallId(symbol) orelse {
                    self.status = .fault;
                    self.faultReason = "Unknown dynamic import symbol name";
                    return;
                };
                ...
                const origA5 = self.regs[15];
                const origA6 = self.regs[16];
                const origA7 = self.regs[17];
                ...
                if (self.syscallHandler) |handler| {
                    handler(self) catch |err| {
                        ...
                        if (self.status == .fault) {
                            self.faultReason = "Syscall error in dynamic import handler";
                        }
                    };
                } else {
                    self.status = .fault;
                ...
                // Return to RA
                self.pc = @truncate(self.regs[1]);
                self.pcUpdated = true;
            },
            1 => { // FUNC3_SBRK -> heap allocation
                const dst = (inst >> 7) & 0x1F;
                const size_reg = (inst >> 15) & 0x1F;
                const old_heap = self.heap_ptr;
                self.regs[dst] = old_heap;
                self.heap_ptr +%= @truncate(self.regs[size_reg]);
            },
            2 => { // FUNC3_MEMSET -> memory set
                const dest = self.regs[10];
                const value = @as(u8, @truncate(self.regs[11]));
                const count = self.regs[12];

                const slice = self.memory.getSliceMut(dest, count) catch {
                    self.status = .fault;
                    self.faultReason = "Memset out of bounds";
                    return;
                };
                @memset(slice, value);
                self.regs[10] = dest;
            },
            3 => { // FUNC3_HEAP_BASE -> load heap base
                const dst = (inst >> 7) & 0x1F;
                self.regs[dst] = sandbox.heapStart;
            },
            else => {
                self.status = .fault;
                self.faultReason = "Unknown custom opcode funct3 for PolkaVM";
            },
        }
    }
