...
pub const Http3Stream = struct {
    ...
    pub fn send_frame(self: *Http3Stream, frame: Http3Frame) !void {
        ...
        log.debug("send_frame(): Writing {} bytes to QUIC stream {}...\n", .{ written, self.stream.id });
        ...
        while (total_written < written) {
            ...
            if (bytes_written == 0) {
                log.debug("send_frame(): Error! Zero bytes written, possible buffer issue.\n", .{});
                return Error.InvalidHttpFrame;
            }

            log.debug("send_frame(): Wrote {} bytes (remaining {})...\n", .{ bytes_written, written - total_written });
            total_written += bytes_written;
        }
    }

    pub fn receive_frame(self: *Http3Stream) !Http3Frame {
        log.debug("Receiving HTTP/3 frame on QUIC stream {}...\n", .{self.stream.id});

        const payload = self.stream.read() catch |err| {
            log.debug("Error: Failed to read payload: {}\n", .{err});
            return err;
        };

        log.debug("Raw Payload Received (len {}): {any}\n", .{ payload.len, payload });
