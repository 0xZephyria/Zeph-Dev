...
pub const Transport = struct {
    ...
    fn setupHttp2Server(self: *Transport) !void {
        ...
        // Parse SETTINGS frame: each parameter is 6 bytes (u16 ID + u32 value)
        ...
        if (settings_length > 0) {
            ...
            // Process each 6-byte SETTINGS parameter
            var offset: usize = 0;
            while (offset + 6 <= settings_payload.len) {
                const param_id = (@as(u16, settings_payload[offset]) << 8) | @as(u16, settings_payload[offset + 1]);
                const param_val = (@as(u32, settings_payload[offset + 2]) << 24) |
                    (@as(u32, settings_payload[offset + 3]) << 16) |
                    (@as(u32, settings_payload[offset + 4]) << 8) |
                    @as(u32, settings_payload[offset + 5]);
                // Apply settings: HEADER_TABLE_SIZE(1), MAX_CONCURRENT_STREAMS(3),
