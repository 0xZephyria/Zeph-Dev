...
pub const Context = struct {
    ...
    rateLimitMutex: std.Thread.Mutex,
    ...
    pub fn init(
        ...
        pool: *core.dag_mempool.DAGMempool,
        ...
        self.* = .{
            ...
            .waitGroup = .{},
            .tcp = tcp_server,
            .readBufferSize = MAX_HEADER_BUFFER_SIZE,
            .handler = handler,
    ...
    pub fn setP2P(self: *Context, p2pServer: *p2p.Server) void {
        self.handler.setP2p(p2pServer);
    }
    ...
    pub fn deinit(self: *Context) void {
        ...
        // Close the TCP listener first to unblock accept() in serveLoop
        self.tcp.deinit();
    ...
    pub fn checkRateLimit(self: *Context, addr: std.net.Address) bool {
...
fn serveLoop(ctx: *Context) void {
    ...
    while (ctx.running.load(.seq_cst)) {
        ...
        const thread = std.Thread.spawn(.{}, handleConnection, .{ ctx, conn }) catch |err| {
...
fn handleConnection(ctx: *Context, conn: std.net.Server.Connection) void {
    defer conn.stream.close();

    // Rate limit check on connection open
    if (!ctx.checkRateLimit(conn.address)) {
    ...
    while (requests_served < MAX_REQUESTS_PER_CONN) {
        ...
        if (requests_served > 0 and !ctx.checkRateLimit(conn.address)) {
            _ = conn.stream.write("HTTP/1.1 429 Too Many Requests\r\nConnection: close\r\n\r\n") catch {};
            return;
        }
        ...
        handleRequestRaw(ctx, arena.allocator(), conn, body[0..body_read], keep_alive) catch |err| {
...
fn handleRequestRaw(ctx: *Context, allocator: std.mem.Allocator, conn: std.net.Server.Connection, body: []const u8, keep_alive: bool) !void {
    // Parse JSON-RPC
    var parsed = std.json.parseFromSlice(std.json.Value, allocator, body, .{}) catch {
        try respondRaw(conn, 200, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32700,\"message\":\"Parse error\"},\"id\":null}", keep_alive);
        return;
    };
    defer parsed.deinit();
    ...
    if (root == .array) {
        ...
        for (root.array.items, 0..) |item, idx| {
            ...
            try processSingleRequest(ctx, allocator, item, batch_writer);
        }

        try batch_writer.writeByte(']');
        try respondRaw(conn, 200, batch_response.items, keep_alive);
        return;
    }

    if (root != .object) {
        try respondRaw(conn, 200, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32600,\"message\":\"Invalid Request\"},\"id\":null}", keep_alive);
        return;
    }
    ...
    try processSingleRequest(ctx, allocator, root, writer);
    try respondRaw(conn, 200, response_json.items, keep_alive);
}

fn processSingleRequest(ctx: *Context, allocator: std.mem.Allocator, root: std.json.Value, writer: anytype) !void {
    if (root != .object) {
    ...
    if (root.object.get("jsonrpc")) |val| {
        if (val != .string or !std.mem.eql(u8, val.string, "2.0")) {
            try writeErrorResponse(writer, id, -32600, "Invalid Request");
            return;
        }
    }
    ...
    const result = ctx.handler.handleRequest(allocator, method_str, params orelse .null) catch |err| {
        if (err == error.MethodNotFound) {
            log.debug("[RPC] Method not found: '{s}'\n", .{method_str});
            try writeErrorResponse(writer, id, -32601, "Method not found");
            return;
        }

        if (err == error.InvalidParams) {
            log.debug("[RPC] Invalid params for '{s}'\n", .{method_str});
            try writeErrorResponse(writer, id, -32602, "Invalid params");
            return;
        }
...
fn writeErrorResponse(writer: anytype, id: ?std.json.Value, code: i32, msg: []const u8) !void {
    try writer.writeAll("{\"jsonrpc\":\"2.0\",\"id\":");
    if (id) |i| {
        try dumpJson(i, writer);
    } else {
...
fn respondRaw(conn: std.net.Server.Connection, status: usize, json_body: []const u8, keep_alive: bool) !void {
