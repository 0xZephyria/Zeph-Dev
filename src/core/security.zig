...
pub const RateLimiter = struct {
    ...
    const Bucket = struct {
        tokens: u32,
        lastRefill: i64, // timestamp in seconds
        lastTxNs: i128, // nanosecond timestamp of last TX

        fn refill(self: *Bucket, capacity: u32, rate: u32, now: i64) void {
            const elapsed = now - self.lastRefill;
            if (elapsed <= 0) return;
    ...
    pub fn checkAndConsume(self: *RateLimiter, sender: types.Address) !void {
        ...
        // 1. Global rate check
        self.globalBucket.refill(self.config.globalCapacity, self.config.globalRefill, now);
        if (!self.globalBucket.consume()) return error.GlobalRateLimited;

        // 2. Per-account rate check
        const gop = try self.buckets.getOrPut(sender);
        if (!gop.found_existing) {
            gop.value_ptr.* = .{
                .tokens = self.config.perAccountCapacity,
                .lastRefill = now,
                .lastTxNs = 0,
            };
        }
        ...
        bucket.refill(self.config.perAccountCapacity, self.config.perAccountRefill, now);

        // 3. Minimum interval check
        if (bucket.lastTxNs > 0) {
            const elapsed_ns = now_ns - bucket.lastTxNs;
            if (elapsed_ns < self.config.minTxIntervalNs) return error.TooFrequent;
        }

        if (!bucket.consume()) return error.AccountRateLimited;
        bucket.lastTxNs = now_ns;
    }
};
...
pub const ReentrancyGuard = struct {
    ...
    pub fn enter(self: *ReentrancyGuard) !void {
        if (self.depth >= self.maxDepth) return error.MaxCallDepthExceeded;
        self.depth += 1;
    }
...
pub const ExecutionTimer = struct {
    ...
    pub fn start(timeout_ms: u64) ExecutionTimer {
        return .{
            ...
            .timeoutNs = @as(i128, timeout_ms) * 1_000_000,
        };
    }

    /// Check if execution has exceeded the timeout.
    pub fn isExpired(self: *const ExecutionTimer) bool {
        const elapsed = std.time.nanoTimestamp() - self.startNs;
        return elapsed > self.timeoutNs;
    }
...
pub const SecurityConfig = struct {
    ...
    maxPoolSize: u32 = 50_000,
    /// Maximum pending TXs per account
    maxPerAccount: u32 = 64,
    /// Bloom filter size (bits) for dedup
    bloomSize: u32 = 1_048_576, // 1M bits = 128KB
    /// Execution timeout in milliseconds
    executionTimeoutMs: u64 = 5_000, // 5 seconds
    /// Maximum call depth
